...
function IPointsDisplay:CreatePoints()
    local pointMult = IPointsDisplay.max_combopoints

    if (IPointsDisplay.powerType == 'ARCANE_CHARGES') then
        pointMult = 5
    end

    local pointWidth = ((CONSTS.SIZES.PLAYER_FRAME.W - 40) / pointMult)
    ...
    for i = 1, IPointsDisplay.max_combopoints do
        local pointFrame = d3ui_PointsDisplay.points[i]
        if (not pointFrame) then
            pointFrame = CreateFrame("StatusBar", nil, d3ui_PointsDisplay, BackdropTemplateMixin and "BackdropTemplate")
        end
...
function IPointsDisplay:HidePoints()
    for i = 1, IPointsDisplay.max_combopoints do
    ...
    if (IPointsDisplay.specialMode == 'RUNE') then
        ...
        for i = 1, IPointsDisplay.max_combopoints do
            d3ui_PointsDisplay.points[i]:HookScript("OnUpdate", THROTTLER:Hook('update_runes', function()
                if (d3ui_PointsDisplay.filledRunes == 6) then
                    return
                end ;
...
function IPointsDisplay:Prep()
    local classId, specialMode = CheckIfHasBalls()

    if (not classId) then
        IPointsDisplay.powerId = nil
        IPointsDisplay.powerType = nil
        IPointsDisplay.specialMode = nil
        return ;
    end
...
function IPointsDisplay:Load ()
    local proceed = IPointsDisplay:Prep()
    if (not proceed) then
        return
    end ;
